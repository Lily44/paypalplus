    ...
    if(osc_get_preference('allow_premium_plus', 'paypalplus')) {
        ...
        if($item) {
            ...
            if(!paypalplus_is_premium_plus($item['pk_i_id'])) {
                ...
                $conn = getConnection();
                $ppl_category = $conn->osc_dbFetchResult("SELECT f_premium_plus_cost FROM %st_paypal_prices WHERE fk_i_category_id = %d", DB_TABLE_PREFIX, $item['fk_i_category_id']);
                ...
                if($category_fee > 0) {
                ?>
                <h1><?php _e('Make the ad premium Plus', 'paypalplus'); ?></h1>
                <div>
                    ...
                    <div style="float:left; width: 50%;">
                        ...
                        <?php if(osc_is_web_user_logged_in()) {
								...
								if( $totalcredit>=$category_fee) {
                                    wallet_button($category_fee, sprintf(__("Premium fee for item %d at %s", "paypalplus"), $item['pk_i_id'], osc_page_title()), $item['fk_i_user_id']."|".$item['pk_i_id']."|".$item['s_contact_email'], "301x".$item['fk_i_category_id']."x".$item['pk_i_id']);
                                } else {
                                    paypalplus_button($category_fee, sprintf(__("Premium fee for item %d at %s", "paypalplus"), $item['pk_i_id'], osc_page_title()), $item['fk_i_user_id']."|".$item['pk_i_id']."|".$item['s_contact_email'], "301x".$item['fk_i_category_id']."x".$item['pk_i_id']);
                                }
                            } else {
                                paypalplus_button($category_fee, sprintf(__("Premium fee for item %d at %s", "paypalplus"), $item['pk_i_id'], osc_page_title()), $item['fk_i_user_id']."|".$item['pk_i_id']."|".$item['s_contact_email'], "301x".$item['fk_i_category_id']."x".$item['pk_i_id']);
                            } ?>
                ...
                <h1><?php _e('There was an error', 'paypalplus'); ?></h1>
                <div>
                    <p><?php _e('The item is already a premium Plus ad', 'paypalplus'); ?></p>
                </div>
                <?php
            }
        } else {
        ...
        <h1><?php _e("There was an error", "paypalplus");?></h1>
        <br/>
        <div><p><?php _e("Premiums Plus ads are not allowed", "paypalplus");?></p></div>
        <?php
    }
