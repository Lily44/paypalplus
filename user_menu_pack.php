...
<div class="content user_account">
    ...
    <?php paypalplus_button($pack, sprintf(__("Credit for %s %s at %s", "paypalplus"), $pack, osc_get_preference("currency", "paypalplus"), osc_page_title()), $user['pk_i_id'] . "|dash|" . $user['s_email'], "401x" . $pack); ?>
        ...
        <?php
        if ($morecredit == '1')
            echo _e('You can get more Bonus Credit after using all you actual bonus!', 'paypalplus');
        else {
            if (osc_get_preference("allow_bonus", "paypalplus") == '1')
                echo '*' . __('Bonus Credit is not cumulative.', 'paypalplus');
            if (osc_get_preference("allow_bonus_expiration", "paypalplus") == '1')
                echo ' ' . sprintf(__('Your Bonus Credit will expire in %d days', 'paypalplus'), osc_get_preference("bonus_days", "paypalplus"));
        }
        ?>
