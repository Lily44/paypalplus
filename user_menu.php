...
// Get user credit balance if applicable.
...
// Get bonus expiration if applicable.
...
<div class="content user_account">
    ...
    <div id="main">
        ...
        <?php if ($bonusamount > '0') { ?>
            <h2 style="color:#060"> <?php
            echo sprintf(__('Your current Bonus credit is %.2f %s', 'paypalplus'), $bonusamount, osc_get_preference('currency', 'paypalplus'));
            if ($diff > '0')
                            ...
                            <?php if (( paypalplus_item_is_enabled() == '1') || (osc_get_preference("auto_enable", "paypalplus") == "1")) { ?>
                                ...
                                <?php if (osc_get_preference("allow_premium", "paypalplus") == "1") { ?>
                                    <?php if (paypalplus_is_premium(osc_item_id())) { ?> 
                                        <strong><?php _e('Already premium', 'paypalplus');
                        echo ' ' . paypalplus_premium_days(osc_item_id());
                        _e('days!', 'paypalplus'); ?></strong>
                                    <?php } else {
                                        if (!paypalplus_is_premium_plus(osc_item_id())) {
                                            ?>
                                            <strong><a href="<?php echo osc_render_file_url(osc_plugin_folder(__FILE__) . "makepremium.php&itemId=" . osc_item_id()); ?>"><?php _e('Make premium', 'paypalplus');
                            echo ' ' . osc_get_preference("premium_days", "paypalplus");
                            _e('days!', 'paypalplus'); ?></a></strong>
                                        <?php }
                                    } ?>
                                <?php } ?>
                        ...
                        _e('days!', 'paypalplus'); ?></strong>
                                    <?php } else {
                                        if (!paypalplus_is_premium(osc_item_id())) {
                                            ?>													 
                                            <strong><a href="<?php echo osc_render_file_url(osc_plugin_folder(__FILE__) . "makepremiump.php&itemId=" . osc_item_id()); ?>"> | <?php _e('Make premium', 'paypalplus');
                            echo ' ' . osc_get_preference("premium_plus_days", "paypalplus");
                            echo _e('days!', 'paypalplus'); ?></a></strong>
                                        <?php } ?>                                                  
                    <?php } ?>
                <?php } ?>
